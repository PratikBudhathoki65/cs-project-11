(function () {
  ...
  const API_KEY = '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';
...
- Your name is Curiosity. Introduce yourself ONLY when:
  1. It's the very first message / greeting (like "hi", "hello", "hey")
  2. Someone directly asks your name or who you are
  3. Someone asks "how are you" — respond naturally and warmly
...
- Never start every message with your name. Talk like a natural, smart friend.`;
  ...
  function saveCurrentChat() {
    ...
    saveTimer = setTimeout(() => {
      ...
      const title = first
        ? first.content.slice(0, 45) || 'Image message'
        : 'New Chat';
      ...
      try {
        ...
        /* Storage full — delete oldest chat */
        const old = Object.values(chatSessions)
  ...
  function loadChat(id) {
    ...
    currentMessages.forEach(m =>
      renderMessage(m.role, m.content, [], m.timestamp, false)
    );
    scrollToBottom();
    updateHistoryUI();
  }
  ...
  function updateHistoryUI() {
    const list = Object.values(chatSessions)
      .sort((a, b) => b.timestamp - a.timestamp)
      .slice(0, 25);

    if (!list.length) {
      DOM.historyList.innerHTML =
        '<p style="padding:.5rem .9rem;font-size:.73rem;' +
        'color:var(--clr-text-muted);opacity:.6">No chats yet</p>';
      return;
    }

    DOM.historyList.innerHTML = list.map(s => `
      <div class="history-item${s.id === currentChatId ? ' active' : ''}"
           data-id="${s.id}">
        <svg class="history-item-icon" viewBox="0 0 24 24"
        ...
        <button class="history-item-del" data-del="${s.id}"
                title="Delete">
    ...
    DOM.historyList.querySelectorAll('.history-item')
      .forEach(item => {
    ...
    DOM.historyList.querySelectorAll('.history-item-del')
      .forEach(btn => {
  ...
  async function sendMessage() {
    ...
    renderMessage(
      'user', text, userMsg.images, userMsg.timestamp, true
    );
  ...
  /* ─────────────────────────────────────────────
     GROQ API CALL — Vision enabled
     ───────────────────────────────────────────── */
  async function callGroq() {
    const messages = [
      { role: 'system', content: SYSTEM_PROMPT },

      ...currentMessages.slice(-12).map(m => {

        /* ── USER message ── */
        if (m.role === 'user') {

          /* User sent images along with (or without) text */
          if (m.images && m.images.length > 0) {

            const contentParts = [];

            /* Add text part first if exists */
  ...
  function renderMessage(role, content, images, timestamp, animate) {
    ...
    /* Image thumbnails (user sent) */
    let imgs = '';
    if (images?.length) {
      imgs = `<div class="user-images">
        ${images.map(i =>
          `<img class="user-img-thumb"
    ...
    const aiActions = `
      <div class="msg-actions">
        ...
        <button class="msg-action-btn speak-btn">
          ${spkIcon} Speak
        </button>
    ...
    /* Videos button */
    row.querySelector('.videos-btn')
      ?.addEventListener('click', () => {
        const last = currentMessages
          .filter(m => m.role === 'user')
          .slice(-1)[0];
        const q = (last?.content || content)
          .slice(0, 60)
          .replace(/[*#_`]/g, '')
          .trim();
        window.open(
          `videos.html?q=${encodeURIComponent(q)}`,
          '_blank'
        );
      });
  ...
  DOM.imageInput?.addEventListener('change', async e => {
    for (const f of Array.from(e.target.files).slice(0, 4)) {
      ...
      if (f.size > 5 * 1024 * 1024) {
        showToast('Max 5MB per image', 'error');
        continue;
      }
  ...
  function renderImagePreviews() {
    ...
    DOM.imagePreviewList.querySelectorAll('.preview-remove')
      .forEach(b => {
        b.addEventListener('click', () => {
          pendingImages.splice(+b.dataset.idx, 1);
          renderImagePreviews();
          updateSendBtn();
        });
      });
  ...
  function updateSendBtn() {
    DOM.sendBtn.disabled =
      (!DOM.chatInput.value.trim() && !pendingImages.length) ||
      isGenerating;
  }
  ...
  DOM.chatInput.addEventListener('input', () => {
    ...
    DOM.chatInput.style.height =
      Math.min(DOM.chatInput.scrollHeight, 150) + 'px';
    updateCharCount();
    updateSendBtn();
  });
  ...
  function formatMarkdown(text) {
    ...
    let h = text.replace(
      /```[\w]*\n?([\s\S]*?)```/g,
      (_, c) => {
        cb.push(`<pre><code>${escHTML(c.trim())}</code></pre>`);
        return `%%CB${cb.length - 1}%%`;
      }
    );

    h = h.replace(/`([^`]+)`/g, (_, c) => {
      ic.push(`<code>${escHTML(c)}</code>`);
    ...
    h = h.replace(
      /Formula: (.+)/g,
      '<div class="formula-block">📐 <strong>Formula:</strong> $1</div>'
    );
