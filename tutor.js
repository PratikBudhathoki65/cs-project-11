(function () {
  ...
  const API_KEY = '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';
  ...
  const SYSTEM_PROMPT = `You are CoreDeck AI Tutor, a friendly and helpful study assistant for Class 11 students in Nepal and India (NEB/CBSE syllabus). Give clear, step-by-step explanations. Use bullet points and simple language. For formulas write "Formula: [formula]". Be encouraging and concise.`;

  function init() {
    try { chatSessions = JSON.parse(localStorage.getItem('cd_chats') || '{}'); }
  ...
  function saveCurrentChat() {
    ...
    saveTimer = setTimeout(() => {
      ...
      const title = first ? first.content.slice(0, 45) : 'New Chat';
      chatSessions[currentChatId] = {
        id: currentChatId, title,
        messages: currentMessages.map(m => ({ role: m.role, content: m.content, timestamp: m.timestamp })),
        timestamp: Date.now()
      };
      try { localStorage.setItem('cd_chats', JSON.stringify(chatSessions)); }
      catch (e) {
  ...
  function loadChat(id) {
    ...
    DOM.messages.appendChild(DOM.typingIndicator);
    DOM.typingIndicator.style.display = 'none';
    currentMessages.forEach(m => renderMessage(m.role, m.content, [], m.timestamp, false));
    scrollToBottom();
    updateHistoryUI();
  }
  ...
  function updateHistoryUI() {
    ...
    if (!list.length) {
      DOM.historyList.innerHTML = '<p style="padding:.5rem .9rem;font-size:.73rem;color:var(--clr-text-muted);opacity:.6">No chats yet</p>';
      return;
    }
  ...
  async function sendMessage() {
    ...
    try {
      ...
      DOM.typingIndicator.style.display = 'none';
      renderMessage('ai', `Something went wrong. Please try again.\n\n_${escHTML(err.message)}_`, [], Date.now(), true);
    }
  ...
  async function callGroq() {
    const messages = [
      ...
      ...currentMessages.slice(-12).map(m => ({ role: m.role === 'user' ? 'user' : 'assistant', content: m.content || '' }))
    ];
    const resp = await fetch(API_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${API_KEY}` },
      body: JSON.stringify({ model: MODEL, messages, max_tokens: 1500, temperature: 0.7, top_p: 0.9, stream: false })
    });
    if (!resp.ok) {
      const e = await resp.json().catch(() => ({}));
      const m = e?.error?.message || `HTTP ${resp.status}`;
      if (resp.status === 401) throw new Error('Invalid API key.');
      if (resp.status === 429) throw new Error('Rate limit. Please wait 30 seconds.');
      if (resp.status === 503) throw new Error('Service busy. Try again.');
      throw new Error(m);
    }
  ...
  function renderMessage(role, content, images, timestamp, animate) {
    ...
    const avatar = role === 'user'
      ...
      : `<div class="msg-avatar ai"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M9 10c0-2 1.5-3 3-3s3 1 3 3c0 1.5-.8 2.5-2 3v1"/><circle cx="12" cy="17" r=".8" fill="currentColor" stroke="none"/></svg></div>`;

    let imgs = '';
    if (images?.length) {
      imgs = `<div class="user-images">${images.map(i => `<img class="user-img-thumb" src="data:${i.mimeType};base64,${i.base64}" alt="img"/>`).join('')}</div>`;
    }

    const body = role === 'ai'
      ? formatMarkdown(content)
      : `<p style="margin:0">${escHTML(content).replace(/\n/g, '<br>')}</p>`;
    ...
    const aiActions = `
      <div class="msg-actions">
        ...
        <button class="msg-action-btn videos-btn">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="3"/><polygon points="10 8 16 12 10 16"/></svg> Videos
        </button>
      </div>`;
  ...
  // ═══════════════════════════════════════════
  // SPEAK — English, direct, no dropdown
  // ═══════════════════════════════════════════
  ...
  function speakEnglish(text, btnEl) {
    ...
    if (!s) { showToast('Voice not supported', 'error'); return; }
    ...
    if (isSpeaking) {
      ...
      document.querySelectorAll('.speak-btn').forEach(b => {
        ...
        b.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 5L6 9H2v6h4l5 4V5z"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg> Speak`;
      });
      return;
    }

    const clean = cleanForSpeech(text);
    if (!clean) { showToast('Nothing to read', 'error'); return; }
    ...
    const trySpeak = () => {
      ...
      u.lang = 'en-US'; u.rate = 0.88; u.pitch = 1.0; u.volume = 1;

      u.onstart = () => {
        isSpeaking = true;
        btnEl.classList.add('speaking');
        btnEl.innerHTML = `<svg viewBox="0 0 24 24" fill="currentColor" width="11" height="11"><rect x="4" y="4" width="16" height="16" rx="2"/></svg> Stop`;
      };
      u.onend = u.onerror = () => {
        isSpeaking = false;
        btnEl.classList.remove('speaking');
        btnEl.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 5L6 9H2v6h4l5 4V5z"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg> Speak`;
      };
      s.speak(u);
    };

    if (s.getVoices().length === 0) {
      s.onvoiceschanged = () => setTimeout(trySpeak, 100);
    } else {
  ...
  DOM.imageInput?.addEventListener('change', async e => {
    for (const f of Array.from(e.target.files).slice(0, 4)) {
      ...
      if (f.size > 5 * 1024 * 1024) { showToast('Max 5MB', 'error'); continue; }
  ...
  function renderImagePreviews() {
    ...
    DOM.imagePreviewList.innerHTML = pendingImages.map((img, i) => `
      <div class="preview-item">
        <img class="preview-img" src="data:${img.mimeType};base64,${img.base64}" alt="preview"/>
  ...
  function clearImages() {
    pendingImages = [];
    DOM.imagePreviewRow.style.display = 'none';
    DOM.imagePreviewList.innerHTML = '';
  }

  function updateSendBtn() {
    DOM.sendBtn.disabled = (!DOM.chatInput.value.trim() && !pendingImages.length) || isGenerating;
  }
  ...
  DOM.sidebarOpen?.addEventListener('click', () =>
    DOM.sidebar.classList.contains('collapsed') ? expandSidebar() : collapseSidebar()
  );
  DOM.newChatBtn?.addEventListener('click', () => { startNewChat(); if (window.innerWidth < 900) collapseSidebar(); });

  function scrollToBottom() {
    setTimeout(() => { DOM.messages.scrollTop = DOM.messages.scrollHeight; }, 60);
  }
  ...
  function formatMarkdown(text) {
    if (!text) return '';
    const cb = [], ic = [];
    let h = text.replace(/```[\w]*\n?([\s\S]*?)```/g, (_, c) => { cb.push(`<pre><code>${escHTML(c.trim())}</code></pre>`); return `%%CB${cb.length-1}%%`; });
    ...
    for (const line of lines) {
      const t = line.trim();
      const bl = /^<(h[1-3]|ul|ol|pre|div|hr)/.test(t);
  ...
  function showToast(msg, type = '') {
    ...
    setTimeout(() => {
      t.style.cssText += 'transition:opacity .3s,transform .3s;opacity:0;transform:translateY(10px)';
